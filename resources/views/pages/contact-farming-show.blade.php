                    ...
                    <div class="col-md-12">
                        <div class="info-group">
                            ...
                            @if($record->varieties && $record->varieties->count() > 0)
                                @php
                                    $vars = collect($record->varieties);
                                    $hay = $vars->filter(fn($x) => strtolower(trim($x->seed_variety ?? '')) === 'hay');
                                    $silage = $vars->filter(fn($x) => strtolower(trim($x->seed_variety ?? '')) === 'silage');
                                    $others = $vars->reject(fn($x) => in_array(strtolower(trim($x->seed_variety ?? '')), ['hay','silage']));
                                    $groups = [
                                        'Hay' => $hay,
                                        'Silage' => $silage,
                                                    ...
                                                    <tbody>
                                                        @foreach($group as $v)
                                                            @php
                                                                ...
                                                                $totalSold = collect($v->current_sales ?? [])->sum(fn($s) => (float)($s->quantity_sold ?? 0));
                                                                $remainingTotal = $production - $totalSold;
                                                            @endphp
                                                            @if (!empty($v->current_sales) && count($v->current_sales) > 0)
                                                                @php $cumulativeSold = 0; @endphp
                                                                @foreach($v->current_sales as $index => $s)
                                                                    @php $cumulativeSold += (float)($s->quantity_sold ?? 0); @endphp
                                                                    <tr>
                                                                        @if($index === 0)
                                                                            <td rowspan="{{ count($v->current_sales) }}">
                                                                            ...
                                                                            <td rowspan="{{ count($v->current_sales) }}">{{ number_format($v->production_quantity_mt, 2) }}</td>
                                                                            <td rowspan="{{ count($v->current_sales) }}">{{ $v->production_price_mt ?? '' }}</td>
                                                                            <td rowspan="{{ count($v->current_sales) }}">{{ $v->production_amount ?? 0 }}</td>
                                                                            <td rowspan="{{ count($v->current_sales) }}">{{ $v->handling_loss ?? '' }}</td>
                                                                            <td rowspan="{{ count($v->current_sales) }}">{{ $v->amount_recovery ?? 0 }}</td>
                                                                        @endif

                                                                        <td>{{ number_format((float)($s->quantity_sold ?? 0), 2) }}</td>
                                                                        <td>{{ $s->sale_price ?? '' }}</td>
                                                                        <td>{{ $s->total_amount ?? number_format(((float)$s->quantity_sold * (float)($s->sale_price ?? 0)), 2) }}</td>
                                                            ...
                                                            @endif
                                                            {{-- History toggle --}}
                                                            @if(!empty($v->history_sales) && count($v->history_sales) > 0)
                                                                <tr>
                                                                    <td colspan="11">
                                                                        <a class="btn btn-sm btn-outline-secondary" data-bs-toggle="collapse" href="#history-{{ $v->id }}" role="button" aria-expanded="false" aria-controls="history-{{ $v->id }}">Show History</a>
                                                                        <div class="collapse mt-2" id="history-{{ $v->id }}">
                                                                            <div class="card card-body">
                                                                                <table class="table table-sm table-bordered mb-0">
                                                                                    <thead>
                                                                                        <tr><th>Type</th><th>Qty</th><th>Price</th><th>Amount</th><th>Buyer</th><th>Date</th></tr>
                                                                                    </thead>
                                                                                    <tbody>
                                                                                    @foreach($v->history_sales as $hs)
                                                                                        <tr>
                                                                                            <td>{{ (!empty($hs->parent_sale_id) && $hs->is_reverse) ? 'Reversal' : 'Original' }}</td>
                                                                                            <td>{{ number_format($hs->quantity_sold ?? 0, 2) }}</td>
                                                                                            <td>{{ $hs->sale_price ?? '' }}</td>
                                                                                            <td>{{ $hs->total_amount ?? number_format(((float)($hs->quantity_sold ?? 0) * (float)($hs->sale_price ?? 0)), 2) }}</td>
                                                                                            <td>{{ $hs->buyer_name ?? '' }}</td>
                                                                                            <td>{{ \Carbon\Carbon::parse($hs->created_at)->format('d-m-y') }}</td>
                                                                                        </tr>
                                                                                    @endforeach
                                                                                    </tbody>
                                                                                </table>
