...
<body>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="row mb-3">
                                                    ...
                                                    @foreach($blocks as $block)
                                                        <div class="form-check mb-2">
                                                            <input type="checkbox"
                                                                   ...
                                                                   value="{{ $block->id }}"
                                                                   id="block_{{ $block->id }}"
                                                                   {{ in_array($block->id, old('blocks', [])) ? 'checked' : '' }}
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <div class="mt-3">
                                                <label class="form-label">Total Selected Area</label>
                                                <div id="totalArea" class="form-control-plaintext">0.00 acres</div>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-12">
                                            <label class="form-label">Seed Variety <small class="text-muted">(add multiple)</small></label>

                                            <div id="seedVarietyContainer">
                                                {{-- If old input exists (from validation), render them --}}
                                                @php
                                                    $oldVarieties = old('seed_variety');
                                                    if(is_null($oldVarieties)) {
                                                        $oldVarieties = [''];
                                                    } elseif(!is_array($oldVarieties)) {
                                                        $oldVarieties = [$oldVarieties];
                                                    }
                                                @endphp

                                                <div class="row" id="seedVarietyRow">
                                                @foreach($oldVarieties as $idx => $variety)
                                                    <div class="col-md-3 seed-var-col">
                                                        <div class="input-group mb-2 seed-var-input-group">
                                                            <input type="text" name="seed_variety[]" class="form-control @error('seed_variety.' . $idx) is-invalid @enderror" value="{{ $variety }}" placeholder="Enter seed variety">
                                                            <button type="button" class="btn btn-outline-success btn-add-var" title="Add variety">
                                                                <i class="fas fa-plus"></i>
                                                            </button>
                                                            <button type="button" class="btn btn-outline-danger btn-remove-var" title="Remove variety">
                                                                <i class="fas fa-minus"></i>
                                                            </button>
                                                        </div>
                                                        @error('seed_variety.' . $idx)
                                            ...
                                            <small class="text-muted">Click + to add another variety. Use - to remove a row.</small>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        function calculateTotalArea() {
            ...
            totalAreaElement.textContent = totalArea.toFixed(2) + ' acres';

            // Update the hidden area input
        ...
        // ============= Seed Variety Add/Remove Handlers =============
        function createVarietyCol(value = '') {
            const col = document.createElement('div');
            col.className = 'col-md-3 seed-var-col';

            const inputGroup = document.createElement('div');
            inputGroup.className = 'input-group mb-2 seed-var-input-group';

            const input = document.createElement('input');
            input.type = 'text';
            input.name = 'seed_variety[]';
            input.className = 'form-control';
            input.placeholder = 'Enter seed variety';
            input.value = value;

            const addBtn = document.createElement('button');
            addBtn.type = 'button';
            addBtn.className = 'btn btn-outline-success btn-add-var';
            addBtn.title = 'Add variety';
            addBtn.innerHTML = '<i class="fas fa-plus"></i>';

            const removeBtn = document.createElement('button');
            removeBtn.type = 'button';
            removeBtn.className = 'btn btn-outline-danger btn-remove-var';
            removeBtn.title = 'Remove variety';
        ...
        function addVarietyRow(afterCol = null, value = '') {
            const row = document.getElementById('seedVarietyRow');
            if (!row) return;
            const col = createVarietyCol(value);
            if (afterCol && afterCol.closest && afterCol.closest('#seedVarietyRow')) {
                // insert after the reference column
                if (afterCol.nextSibling) row.insertBefore(col, afterCol.nextSibling);
        ...
        function removeVarietyRow(button) {
            const col = button.closest('.seed-var-col');
            if (!col) return;
            const row = document.getElementById('seedVarietyRow');
            // Prevent removing last column
            if (row.querySelectorAll('.seed-var-col').length <= 1) {
                const input = col.querySelector('input[name="seed_variety[]"]');
        ...
        function updateVarietyButtons() {
            const cols = document.querySelectorAll('#seedVarietyRow .seed-var-col');
            cols.forEach((c, idx) => {
                const removeBtn = c.querySelector('.btn-remove-var');
                if (removeBtn) {
                    removeBtn.style.visibility = cols.length === 1 ? 'hidden' : 'visible';
                }
            });
        }
        ...
        document.addEventListener('click', function(e) {
            const addBtn = e.target.closest('.btn-add-var');
            if (addBtn) {
                e.preventDefault();
                const col = addBtn.closest('.seed-var-col');
                addVarietyRow(col);
                return;
            }
        ...
        document.getElementById('farmingModal').addEventListener('hidden.bs.modal', function () {
            ...
            // Reset seed variety container to a single blank column
            const seedContainer = document.getElementById('seedVarietyContainer');
            if (seedContainer) {
                seedContainer.innerHTML = `
                    <div class="row" id="seedVarietyRow">
                        <div class="col-md-3 seed-var-col">
                            <div class="input-group mb-2 seed-var-input-group">
                                <input type="text" name="seed_variety[]" class="form-control" value="" placeholder="Enter seed variety">
                                <button type="button" class="btn btn-outline-success btn-add-var" title="Add variety"><i class="fas fa-plus"></i></button>
                                <button type="button" class="btn btn-outline-danger btn-remove-var" title="Remove variety"><i class="fas fa-minus"></i></button>
                            </div>
                        </div>
                    </div>
                `;
