        ...
        <div class="main-containerr" style="margin-top: 19px;">
            ...
            <tbody>
                @if(isset($records) && $records->count() > 0)
                    @foreach($records->groupBy('plot_name') as $plot => $plotRecords)
                        {{-- Group header for each plot --}}
                         
        
                        @foreach($plotRecords as $record)
                            <tr>
                                  <td>{{ $record->plot_name ?? '--' }}</td>
                                   <td>{{ $record->block_name ?? '--' }}</td>
                                   <td>{{ $record->season_name ?? '--' }}</td>
                                   <td>{{ $record->start_date ? \Carbon\Carbon::parse($record->start_date)->format('d-m-Y') : '--' }}</td>
                                <td>{{ $record->first_sowing_date 
                                    ? \Carbon\Carbon::parse($record->first_sowing_date)->format('d-m-Y') 
                                    : '--' }}</td>
                            
                            <td>{{ $record->last_sowing_date 
                                    ? \Carbon\Carbon::parse($record->last_sowing_date)->format('d-m-Y') 
                                    : '--' }}</td>
                                
                                <td>{{ $record->first_harvest_date ? \Carbon\Carbon::parse($record->first_harvest_date)->format('d-m-Y') : '--' }}</td>
                                <td>{{ $record->last_harvest_date ? \Carbon\Carbon::parse($record->last_harvest_date)->format('d-m-Y') : '--' }}</td>
                                ...
                                <td>
                                 ...
                                 <td>{{ $record->closed_date ? \Carbon\Carbon::parse($record->closed_date)->format('d-m-Y') : '--' }}</td>
                              <td class="text-center align-middle">
                                  <div class="d-flex justify-content-center gap-3">
                                    ...
                                    @if($record->status == 'Active')
                                      <a href="javascript:void(0)" 
                                         onclick="closeCycle({{ $record->id }})" 
                                         class="text-danger fs-5" 
                                         title="Close Season">
                                         <i class="fas fa-lock"></i>
                                      </a>
                                    @else
                                      <span class="text-success fs-5" title="Closed">
                ...
                @else
                    <tr>
                        <td colspan="10" class="text-center text-muted py-4">
                            ...
                            No season records found. Add your first record above.
                        </td>
                    </tr>
                @endif
            </tbody>
...
function viewRecord(id) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // ✅ Sowing rows
            let sowingRows = sowing.length
                ? sowing.map((row, i) => `
                    <tr>
                        <td>${i + 1}</td>
                        <td>${row.date ?? '--'}</td>
                        <td>${row.seed_name 
                            ? `${row.seed_name} ${row.variety_of_seed ? '(' + row.variety_of_seed + ')' : ''}`
                            : '--'}
                        </td>
                        <td>${row.area_used ?? '--'}</td>
                ...
                `).join('')
                : `<tr><td colspan="5" class="text-center text-muted">No sowing records found</td></tr>`;

            // ✅ Harvesting rows
            let harvestRows = harvesting.length
                ? harvesting.map((row, i) => `
                    <tr>
                        <td>${i + 1}</td>
                        <td>${row.date ?? '--'}</td>
                        <td>${row.area_used ?? '--'}</td>
                ...
                `).join('')
                : `<tr><td colspan="4" class="text-center text-muted">No harvesting records found</td></tr>`;

            // ✅ Horizontal Season Info
            let seasonInfo = `
                <div class="d-flex flex-wrap gap-3 mb-3 border rounded p-3 bg-light">
                    <span><strong>Block:</strong> ${season.block_name ?? '--'}</span>
                    <span><strong>Plot:</strong> ${season.plot_name ?? '--'}</span>
                    <span><strong>Status:</strong> 
                        <span class="badge ${season.status === 'open' ? 'bg-success' : 'bg-secondary'}">
                            ${season.status ?? '--'}
                        </span>
                    </span>
                    <span><strong>Start Date:</strong> ${season.start_date ?? '--'}</span>
                    <span><strong>Closed Date:</strong> ${season.end_date ?? '--'}</span>
                </div>
            `;

            // ✅ Final HTML
            let html = `
                <h5 class="mb-3 text-primary fw-bold">Season Information</h5>
                ${seasonInfo}

                <h6 class="fw-bold text-success">🌱 Sowing Details</h6>
                <table class="table table-sm table-bordered">
                    ...
                    <tbody>${sowingRows}</tbody>
                </table>

                <h6 class="fw-bold text-success mt-3">🌾 Harvesting Details</h6>
                <table class="table table-sm table-bordered">
                    <thead class="table-light">
                        <tr><th>#</th><th>Date</th><th>Area (Acre)</th><th>Yield (MT)</th></tr>
                    </thead>
   ...
   function closeCycle(id) {
    if (!confirm("Are you sure you want to close current season?")) return;

    // read CSRF token from meta tag (works whether JS is inline or external)
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: {}, // _token not required here because header is set
        success: function(response) {
            // backend should return JSON: { message: '...' }
            alert(response.message ?? 'Season closed successfully');
            // either reload or update the UI:
            location.reload();
        },
        error: function(xhr) {
            // try to surface server message, validation errors, or status text
