        ...
        <tbody>
            @forelse ($data as $index => $row)
                ...
                <tr>
                    ...
                    <td>{{ number_format($row->electricity_cost ?? 0, 2) }}</td>
                    ...
                    @php
                        ...
                        if (is_string($row->manpower_category_id)) {
                            $decoded = json_decode($row->manpower_category_id, true);
                            if (is_array($decoded)) {
                                $manpowerRows = $decoded;
                            }
                        }
                        // Case 2: category_id is array
                        elseif (is_array($row->manpower_category_id)) {
                            $manpowerRows = $row->manpower_category_id;
                        }
                        // Case 3: category_id is single INT
                        elseif (is_numeric($row->manpower_category_id)) {
                            $manpowerRows[] = [
                    ...
                    @php
                        ...
                        if (is_string($row->major_maintenance)) {
                            $maintenanceData = json_decode($row->major_maintenance, true);
                        } elseif (is_array($row->major_maintenance)) {
                            $maintenanceData = $row->major_maintenance;
                        }
                    @endphp
                    <td>
                        @if(is_array($maintenanceData))
                            @foreach($maintenanceData as $item)
                                {{ $item['spare_part'] ?? '-' }} {{ number_format($item['value'] ?? 0, 2) }}<br>
                            @endforeach
                        @else
