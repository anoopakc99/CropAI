...
<style>
    ...
    .pagination .page-item.active .page-link {
        ...
        border-color: #3498db;
        color: white;
        box-shadow: 0 44px 12px rgba(52, 152, 219, 0.3);
    }
    ...
    /* Enhanced Fertilizer Display Styles */
    .fertilizer-display {
        max-width: 200px;
        padding: 8px;
    }
    ...
    /* Style for individual maintenance items */
    .maintenance-item {
        display: block;
        margin-bottom: 4px;
        font-size: 0.88rem;
        ...
        <tbody>
            @forelse($records as $index => $record)
                ...
                <tr>
                    ...
                    <td style="white-space: nowrap;">{{ $record->date ? \Carbon\Carbon::parse($record->date)->format('d-m-Y') : '--' }}</td>
                    <td>{{ $record->block_name ?? 'N/A' }}</td>
                    ...
                    <td class="fertilizer-display">
                        @if(!empty($record->formatted_fertilizers))
                            @foreach($record->formatted_fertilizers as $fertilizer)
                            ...
                            @endforeach
                        @else
                            <div class="no-fertilizer">No fertilizer</div>
                        @endif
                    </td>

                    {{-- Fertilizer Used (Quantity) Display --}}
                    <td class="fertilizer-display">
                        @if(!empty($record->formatted_fertilizers))
                            @foreach($record->formatted_fertilizers as $fertilizer)
                                <div class="fertilizer-item">
                                    <span class="fertilizer-quantity">
                                        {{ $fertilizer['quantity'] }} {{ $fertilizer['uom'] }}
                                    </span>
                                </div>
                            @endforeach
                        @else
                            <div class="no-fertilizer">0 kg</div>
                        @endif
                    </td>

                    {{-- Machine Names --}}
                    <td>
                        @if(!empty($record->formatted_machines))
                            
                            <span class="badge bg-success-subtle">
                                        <i class="fas fa-cogs"></i> {{ implode(', ', $record->formatted_machines) }}
                            </span>
                        @else
                            -
                        @endif
                    ...
                    {{-- Tractor Names --}}
                    <td>
                        @if(!empty($record->formatted_tractors))
                           
                             <span class="badge bg-warning-subtle">
                                <i class="fas fa-tractor"></i>  {{ implode(', ', $record->formatted_tractors) }}
                            </span>
                        @else
                            -
                        @endif
                    ...
                    <td>{{ $record->hsd_consumption }}</td>
                    <td>{{ $record->hours_used }}</td>
                    <td>{{ $record->manpower_type ?? '-' }}</td>
                   <td>
                    @php
                        $manpowerRows = [];
                
                        // Case 1: category_id is JSON string
                        if (is_string($record->category_id)) {
                            $decoded = json_decode($record->category_id, true);
                            if (is_array($decoded)) {
                                $manpowerRows = $decoded;
                            }
                        }
                        // Case 2: category_id is array
                        elseif (is_array($record->category_id)) {
                            $manpowerRows = $record->category_id;
                        }
                        // Case 3: category_id is single INT
                        elseif (is_numeric($record->category_id)) {
                            $manpowerRows[] = [
                                'category_id'   => $record->category_id,
                                'no_of_person'  => $record->no_of_person ?? 0
                            ];
                        }
                    @endphp
                    @forelse($manpowerRows as $row1)
                        ...
                        @endphp
                            <span>
                                @if($categoryName)
                               {{ $categoryName }} - {{ $row1['no_of_person'] }}
                               @else
                               -
                               @endif
                            </span><br>
                    ...
                    {{-- Major Maintenance --}}
                    <td>
                        @if(!empty($record->major_maintenance_parsed))
                            <ul style="list-style: none; padding-left: 0; margin: 0;">
                                @foreach($record->major_maintenance_parsed as $part)
                                    <li class="maintenance-item">
                                        <strong>{{ $part['spare_part'] ?? 'N/A' }}</strong>
                                        <span class="price">₹{{ number_format($part['value'] ?? 0, 2) }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        @else
                    ...
                    <td>
    <div class="">
        <i class=""></i> {{ number_format($record->total_cost ?? 0, 2) }}
    </div>
</td>
            ...
            @empty
                <tr>
                    <td colspan="19" class="text-center py-4">No records found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
