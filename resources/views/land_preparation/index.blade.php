...
<style>
    ...
    /* Added for Machine/Tractor Badges */
    .bg-secondary-subtle {
    ...
    /* Added for Major Maintenance Badges */
    .bg-info-subtle {
    ...
    /* Adjust badge margin for multiple badges in a cell */
    .badge-container .badge {
        margin-bottom: 5px; /* Adds space between badges when stacked */
        display: block; /* Makes each badge take its own line */
...
<div class="table-responsive">
    <table class="table table-bordered mt-3">
        <thead>
            ...
            <th>Time (Hrs)</th> <!-- Capitalized and standardized -->
            <th>Manpower Type</th>
            ...
            @forelse ($data as $index => $row)
                ...
                <tr>
                    ...
                    <td>
                        <div class="badge-container">
                            @if ($row->tractor_names)
                                @foreach (explode(',', $row->tractor_names) as $tractor)
                   ...
                   <td>
                    ...
                    @php
                        // Ensure major_maintenance is treated as a string before json_decode
                        $json = is_string($row->major_maintenance) ? $row->major_maintenance : json_encode($row->major_maintenance);
                        ...
                        <div class="badge-container">
                            @if(is_array($maintenanceData))
                                @foreach($maintenanceData as $item)
                                    <span class="badge bg-info-subtle">{{ $item['spare_part'] ?? '-' }} - ₹{{ number_format($item['value'] ?? 0.00, 2) }}</span><br>
                                @endforeach
                            @else
                                <span class="badge bg-info-subtle">No maintenance data</span>
                            @endif
                        </div>
                    </td>
