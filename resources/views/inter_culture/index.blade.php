    ...
    <form method="GET" class="w-100">
        <div class="filter-controls">
            <div class="left-controls">
                ...
                <select id="sessionSelect" name="season_name" class="form-select custom-select-enhanced" onchange="this.form.submit()">
                    <option value="">All Seasons</option>
                    <option value="Kharif" {{ request('session') == 'Kharif' ? 'selected' : '' }}> Kharif </option>
                    <option value="Rabi" {{ request('session') == 'Rabi' ? 'selected' : '' }}> Rabi</option>
                    <option value="Zaid" {{ request('session') == 'Zaid' ? 'selected' : '' }}>` Zaid</option>
                </select>
            </div>
            </div>
...
<div class="table-responsive">
    <div class="table-responsive-container">
       @php
    // Helper function to get names from CSV IDs string
    $getNamesFromIds = function($csvIds, $lookup) {
        if (!$csvIds) return 'N/A';
        $ids = explode(',', $csvIds);
        $names = [];
        foreach ($ids as $id) {
            $id = trim($id);
        ...
        return count($names) ? implode(', ', $names) : 'N/A';
    };
@endphp
        ...
        <table class="table table-bordered">
            <thead>
                ...
                <th>Manpower Type</th>
                <th><div>Unskilled</div>
                </th>
                <th><div>Semi-Skilled 1</div>
                </th>
                <th><div>Semi-Skilled 2</div>
                </th>
            ...
            <tbody>
                @foreach ($query as $index => $row)
                    ...
                    <tr>
                        ...
                        <td>{{ $row->hsd_consumption ?? 0 }}</td> {{-- Changed 'N/A' to 0 --}}
                        ...
                        <td>
                            @php
                                $maintenanceItems = json_decode($row->major_maintenance, true);
                            @endphp

                            @if(is_array($maintenanceItems) && !empty($maintenanceItems))
                                @foreach($maintenanceItems as $item)
                                    {{ $item['spare_part'] ?? 'N/A' }} - ₹{{ number_format($item['value'] ?? 0, 2) }}<br>
                                @endforeach
                            @else
                                0
                            @endif
                        ...
                        <td>{{ $row->total_cost ?? 0 }}</td> {{-- Changed 'N/A' to 0 and added currency symbol --}}
