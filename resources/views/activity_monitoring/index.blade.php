...
<style>
    ...
    /* Image Styling */
    .thumbnail-img {
        height: 100px; /* Set fixed height for thumbnail images */
        width: 100px; /* Optional: set width for square thumbnails */
        object-fit: cover; /* Maintain aspect ratio, crop if necessary */
        border-radius: 8px; /* Match your design's rounded corners */
    ...
    #modalImage {
        max-height: 500px; /* Set maximum height for modal image */
        max-width: 100%; /* Prevent overflow */
        object-fit: contain; /* Show full image without cropping */
        border-radius: 8px; /* Match design */
    }
...
<div class="table-responsive">
    ...
    $dateField = $activity->date ?? $activity->created_at ?? now();
...
@endphp
                <tr>
                    ...
                    <td>{{ $activity->block_name ?? 'N/A' }}</td>
                    <td>{{ $activity->plot_name }}</td>
                    <td>{{ $activity->area }}</td>
                    ...
                    <td>
                        @if($activity->image_path)
                            <img
                                ...
                                alt="Activity Image of {{ $activity->plot_name }}"
                                class="thumbnail-img"
            ...
            @empty
                <tr>
                    <td colspan="7" class="text-center py-4 text-muted">
                        <i class="fas fa-info-circle me-2"></i> No activity data found matching your criteria.
                    </td>
                </tr>
            @endforelse
        </tbody>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="imageModalLabel">Captured Activity Image</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body text-center">
                <img id="modalImage" src="" alt="Full Size Activity Image" class="img-fluid rounded shadow-sm">
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    imageModal.addEventListener('show.bs.modal', function (event) {
        ...
        modalImage.alt = button.alt; // Copy alt text for accessibility
    });
</script>
