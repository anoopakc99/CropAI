...
<table class="table table-bordered">
    <thead>
        <tr>
         <th>Sr. No.</th>
            ...
            <th>Area (Acre)</th>
            <th>Area Covered (Acre)</th> <!-- Corrected spelling -->
            <th>Manual Season</th>
            <th>Seed Name</th>
            <th>Machine Used</th>
            <th>Tractor Used</th>
            <th>HSD Consumption (Ltr)</th> <!-- Added unit -->
            <th>Time (Hrs)</th> <!-- Added unit -->
            <th>Manpower Type</th> <!-- More precise than "Man Power" -->
    ...
    <tbody>
        @forelse ($data as $index => $row)
            @php
                // Determine season based on date - NEW LOGIC
                // Check if date field exists, if not use created_at or other date field
            ...
            <tr>
                ...
                <td>
                    @php
                        $ids = explode(',', $row->machine_id);
                    @endphp
                    <div class="d-flex flex-wrap gap-1">
                        @foreach ($ids as $id)
                            @if (!empty($id) && isset($allMachines[$id]))
                            <span class="badge bg-success-subtle">
                                        <i class="fas fa-cogs"></i> {{ $allMachines[$id] }}
                                    </span>
                ...
                <td>
                    @php
                        $ids = explode(',', $row->tractor_id);
                    @endphp
                    <div class="d-flex flex-wrap gap-1">
                        @foreach ($ids as $id)
                            @if (!empty($id) && isset($allTractors[$id]))
                            <span class="badge bg-warning-subtle">
                                <i class="fas fa-tractor"></i> {{ $allTractors[$id] }}
                            </span>
            ...
            <td>
                @if(isset($item->major_maintenance))
...
<script>
    $(document).ready(function() {
        // Apply Select2 to all dropdowns with enhanced styling - UPDATED
