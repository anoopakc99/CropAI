...
@section('content')
                ...
                <div class="left-controls">
                    @if ($role == 1)
                        <div class="form-group-compact">
                            ...
                            <select name="site_id" id="site_id" class="form-select custom-select-enhanced"
                    ...
                    @else
                        <div class="form-group-compact">
                            ...
                            <input type="text" class="form-control custom-select-enhanced"
                                value="{{ $sites[$selectedSiteId] ?? 'N/A' }}" readonly>
                        </div>
                    @endif
                    ...
                    <div class="form-group-compact">
                        ...
                        <select id="blockSelect" name="block_name" class="form-select custom-select-enhanced"
                            onchange="this.form.submit()">
                    ...
                    <div class="form-group-compact">
                        ...
                        <select name="season_id" class="form-select custom-select-enhanced" onchange="this.form.submit()">
                    ...
                    <div class="text-end mb-3" style="margin-top: 39px;">
                        <a href="{{ route('area_leveling.export', request()->all()) }}" class="btn btn-success">
    ...
    {{-- Rest of your table and pagination code with Block Name column added --}}
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @forelse ($data as $index => $item)
                    {{-- Only show data that matches the selected session filter --}}
                    @php
                        // Determine season based on DB/manual entry
                        ...
                        if ($manualSession) {
                            ...
                            $seasonClass = 'bg-' . strtolower($manualSession);
                        } else {
                            $seasonLabel = '--';
                            $seasonClass = '';
                        }
                    ...
                    <tr>
                        ...
                        <td>
                            @if($seasonLabel !== '--')
                                <span class="badge {{ $seasonClass }}">
                                    <i class="fas fa-seedling"></i> {{ $seasonLabel }}
                                </span>
                            @else
                                --
                            @endif
                        ...
                        <td>
                            @php
                                $seedName = DB::table('seed')->where('id', $item->crop_id)->value('name');
                            @endphp
                            {{ $seedName ?? '--' }}
                        </td>
                        ...
                        <td>{{ $item->area }}</td>
                        <td>{{ $item->area_leveling }}</td>
                        <td>
                            @if (is_array($item->machine_names))
                                <div class="d-flex flex-wrap justify-content-center gap-1">
                                    @foreach ($item->machine_names as $name)
                                        <span class="badge bg-success-subtle">
                                            <i class="fas fa-cogs"></i> {{ trim($name) }}
                                        </span>
                                    @endforeach
                                </div>
                            @elseif (is_string($item->machine_names) && !empty($item->machine_names))
                                <div class="d-flex flex-wrap justify-content-center gap-1">
                                    @foreach (explode(',', $item->machine_names) as $name)
                                        <span class="badge bg-success-subtle">
                                            <i class="fas fa-cogs"></i> {{ trim($name) }}
                                        </span>
                                    @endforeach
                        ...
                        <td>
                            @if (is_array($item->tractor_names))
                                <div class="d-flex flex-wrap justify-content-center gap-1">
                                    @foreach ($item->tractor_names as $name)
                                        <span class="badge bg-warning-subtle">
                                            <i class="fas fa-tractor"></i> {{ trim($name) }}
                                        </span>
                                    @endforeach
                                </div>
                            @elseif (is_string($item->tractor_names) && !empty($item->tractor_names))
                                <div class="d-flex flex-wrap justify-content-center gap-1">
                                    @foreach (explode(',', $item->tractor_names) as $name)
                                        <span class="badge bg-warning-subtle">
                                            <i class="fas fa-tractor"></i> {{ trim($name) }}
                                        </span>
                                    @endforeach
                        ...
                        <td>{{ $item->hsd_consumption }}</td>
                        <td>
                            @if(is_numeric($item->hsd_cost))
                                {{ number_format($item->hsd_cost, 2) }}
                            @else
                                <span class="text-danger">{{ $item->hsd_cost }}</span>
                            @endif
                        </td>
                        <td>{{ $item->hours_used }} Hrs</td>
                        <td>{{ $item->manpower_type }}</td>
                        <td>
                            @php
                                $manpowerRows = [];

                                // Case 1: category_id is JSON string
                                if (is_string($item->category_id)) {
                                    $decoded = json_decode($item->category_id, true);
                                    if (is_array($decoded)) {
                                        $manpowerRows = $decoded;
                                    }
                                }
                                // Case 2: category_id is array
                                elseif (is_array($item->category_id)) {
                                    $manpowerRows = $item->category_id;
                                }
                                // Case 3: category_id is single INT
                                elseif (is_numeric($item->category_id)) {
                                    $manpowerRows[] = [
                                        'category_id' => $item->category_id,
                                        'no_of_person' => $item->no_of_person ?? 0
                                    ];
                                }
                            @endphp
                            @forelse($manpowerRows as $row)
                                ...
                                @endphp
                                <span>
                                    {{ $categoryName ?? 'N/A' }} - {{ $row['no_of_person'] }}
                                </span><br>
                                <!-- <span class="badge bg-secondary-subtle">
                                            {{ $row['no_of_person'] }} persons
                                        </span> -->
                        ...
                        @php
                            $json = is_string($item->major_maintenance) ? $item->major_maintenance : json_encode($item->major_maintenance);

                            $maintenanceData = json_decode($json, true);
                        @endphp
                        <td>
                            @if($item->major_maintenance)
                ...
                @empty
                    <tr>
                        <td colspan="18" class="text-center text-muted py-4">No data found for the selected filter.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <script>
        $(document).ready(function () {
            // Apply Select2 to all dropdowns with enhanced styling
        ...
        function exportToExcel() {
            ...
            // Create download link - Fixed URL
            const exportUrl = window.location.origin + window.location.pathname.replace('/area-leveling', '') + '/export?' + params.toString();

            // Create invisible link and trigger download
            const link = document.createElement('a');
            link.href = exportUrl;
            link.download = `area_leveling_data_${new Date().toISOString().split('T')[0]}.csv`;
