...
public function store(StoreContractFarmingRequest $request)
{
    try {
        if (!Auth::check()) {
        ...
        $cf = DB::table('contact_farming')->insert([
        ...
        foreach ($validated['variety'] as $index => $seedVarietyName) {
            ...
            if (isset($validated['sales'][$index])) {
                ...
                for ($i = 0; $i < $num; $i++) {
                    ...
                    // `total_amount` is a GENERATED column in MySQL, do not insert it explicitly
                    $parentId = $salesData['parent_sale_id'][$i] ?? 0;
                    $isRev = isset($salesData['is_reverse'][$i]) ? (int)$salesData['is_reverse'][$i] : 0;
                    // Ensure parent_sale_id is 0 for normal sales. For reverse sales a valid parent id must be provided.
                    if ($isRev && empty($parentId)) {
                        // skip inserting a reverse record without a valid parent reference
                        continue;
                    }

                    $variety->sales()->create([
                        'quantity_sold' => $qty,
                        'sale_price' => $price,
                        'buyer_name' => $buyer,
                        'sale_date' => $date,
...
foreach ($record->varieties as $variety) {
    ...
    $originals = $allSales->filter(fn($s) => empty($s->parent_sale_id) || $s->parent_sale_id == 0)->values();
    ...
    public function edit($id)
    {
        try {
            ...
            foreach ($varieties as $v) {
                $sales = DB::table('contact_farming_sales')->where('variety_id', $v->id)->orderBy('id')->get();
