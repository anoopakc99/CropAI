        ...
        try {
            ...
            foreach ($plots as $plot) {
                ...
                if (empty($plot->lattitude) || empty($plot->longitude)) {
                    $this->warn("⚠ Skipping Plot ID: {$plot->plot_id} due to missing coordinates.");
                    Log::warning("Skipping plot fetch: Missing coordinates", ['plot_id' => $plot->plot_id]);
                    $failureCount++;
                    continue;
                }
                ...
                while ($attempt < $MAX_ATTEMPTS && !$shouldContinuePlot) {
                    ...
                    try {
                        ...
                        // Rate limit handling inside taskResult error string
                        if (isset($taskResult['error'])) {
                            $errorMsg = strtolower($taskResult['error']);
                            if (str_contains($errorMsg, 'requests limit exceeded') && str_contains($errorMsg, '1000')) {
                                $this->error("❌ CRITICAL: Daily limit of 1000 requests exceeded. Aborting all remaining plots.");
                                return 1; // Stop the entire command
                            }
                            
                            if (str_contains($errorMsg, 'limit exceeded') || str_contains($errorMsg, '429')) {
                                $this->warn("⚠ Rate-Limit Hit for Plot {$plot->plot_id} (attempt {$attempt}). Waiting 60 seconds...");
                                sleep(60);
                        ...
                        if ($status == 429) {
                            $this->warn("⚠ 429 Rate-Limit Hit for Plot {$plot->plot_id} (attempt {$attempt}). Waiting 60 seconds...");
                            sleep(60);
                            continue;
                        }
                        ...
                        if ($respStatus == 429) {
                            $this->warn("⚠ 429 while fetching result for Plot {$plot->plot_id} (attempt {$attempt}). Waiting 60 seconds...");
                            sleep(60);
                            continue;
                        }
