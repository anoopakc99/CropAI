...
$apiKey = env('EOS_API_KEY');
    ...
    $payload = [
        ...
        "params" => [
            ...
            "geometry" => [
                "type" => "Polygon",
                "coordinates" => $polygon   // ✅ pass directly, no extra []
            ],
            "reference" => $reference,
 ...
 */
public function getNDVITaskResult($taskId, $maxAttempts = 10, $waitSeconds = 15)
{
    try {
        if (!$taskId) {
            return [
        ...
        do {
            ...
            // Check if task is complete with results
            if ($status === 'completed' && isset($data['result'])) {
                if (is_array($data['result']) && count($data['result']) > 0) {
            ...
            // Task failed or in unknown state
            if ($status === 'failed') {
                return [
                    'success' => false,
